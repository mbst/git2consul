  ...
  var handle_expanded_keys_with_different_file_types = function() {
    if (file.endsWith('.json')) {
      ...
      file_deleted(branch, file, function (err) {
        ...
        var json = JSON.stringify(YAML.load(fqf));
        var yamlFqf = fqf.substring(0, fqf.length - 5) + '.yaml';
        fs.writeFile(yamlFqf, json, function(err) {
           if (err) throw err;
           file_deleted(branch, file, function (err) {
               if (err) return cb('Failed to delete key ' + key_name + ' due to ' + err);
               handle_json_kv_file(yamlFqf, cb);
           });
        });
